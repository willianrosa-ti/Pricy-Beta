...
// 1. (NOVO) Importa o useNavigate para redirecionar
import { Link, useNavigate } from 'react-router-dom';

// 2. (NOVO) Define a URL da sua API
...
function PaginaLogin() {
  ...
  // 3. (NOVO) Estado de carregamento e hook de navegação
  const [isLoading, setIsLoading] = useState(false);
  const navigate = useNavigate(); // Hook para redirecionar

  // 4. (ATUALIZADO) Função de submissão com lógica de API
  const aoSubmeterFormulario = async (evento) => {
    ...
    try {
        ...
        if (response.ok) { // Status 200-299 (Sucesso)
            ...
            // 5. (NOVO) Salva o Token e os dados do usuário no localStorage
            localStorage.setItem('pryce_token', data.token);
            localStorage.setItem('pryce_user', JSON.stringify(data.usuario));

            // 6. (NOVO) Redireciona para o painel de controle (dashboard)
            navigate('/dashboard'); 

        } else { // Status 4xx, 5xx (Erro)
            // Exibe o erro da API (ex: "Email ou senha inválidos.")
  ...
  // Funções que limpam o erro ao digitar (Seu código - Mantido)
      ...
      <form onSubmit={aoSubmeterFormulario}>
        ...
        <div className="form-grupo">
          ...
          <input 
            ...
            required 
            disabled={isLoading} // Desabilita durante o load
          />
        </div>
        ...
        {/* 7. (ATUALIZADO) Botão agora reage ao estado 'isLoading' */}
